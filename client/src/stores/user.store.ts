    ...
    actions: {
        ...
        async enterSite() {
            ...
            if (userLoginTime && "_id" in this.getUser) {
                ...
                await axios.post(`http://localhost:3000/enter-site`, {userId: this.getUser._id, onlineId: this.getUser.onlineId})
                ...
                if (new Date().getTime() < (userLoginLocalStorageTime * 60 * 60 * 24)) {
                    this.clearUserLocal()
                    const resp = await axios.post(`http://localhost:3000/enter-site`, {}) // ITempUser döner.
        ...
        async fetchAllUsers() {
            const resp = await axios.get(`http://localhost:3000/users`, {headers: {Authorization: `Bearer ${this.getUser.accessToken}`}})
            console.log(resp)
        },
        ...
        async login(body: { email: string, password: string }) {
            try {
                ...
                const resp = await axios.post(`http://localhost/users/login`, {...body, onlineId: this.getUser.onlineId})
                if (resp.data) {
                    this.setUser(resp.data)
                    this.localizeUser()
        ...
        async register(body: { name: string, lastName: string, email: string, password: string, role: string }) {
            try {
                const resp = await axios.post(`http://localhost:3000/users/register`, {...body, onlineId: this.getUser.onlineId})
                if (resp.data) {
                    this.setUser(resp.data)
                    await router.replace({name: 'home'})
                }
        ...
        async logout() {
            const resp = await axios.post(`http://localhost:3000/users/logout`, {onlineId: this.getUser.onlineId})
            if(resp.data) {
                this.setUser(resp.data)
                this.clearUserLocal()
            }
